...
@section('title', $makeup->name . ' - Portfolio')

@section('content')
<div class="min-h-screen bg-[#FAF9F6] py-12">
    <div class="container mx-auto px-4">
        <!-- En-tête avec informations du maquilleur -->
        <div class="flex items-center mb-4">
            ...
            <h1 class="text-3xl font-bold text-[#333333] font-display">Portfolio de {{ $makeup->name }}</h1>
        </div>
        ...
        <!-- Profil du maquilleur -->
        <div class="bg-white rounded-xl shadow-md p-6 mb-8">
            <div class="flex flex-col md:flex-row gap-6">
                <div class="md:w-1/4">
                    @if($makeup->photo)
                        <img src="{{ asset('storage/' . $makeup->photo) }}" alt="{{ $makeup->name }}" class="w-full h-48 md:h-64 object-cover rounded-lg">
                    @else
                ...
                <div class="md:w-3/4">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h2 class="text-2xl font-semibold text-[#333333]">{{ $makeup->name }}</h2>
                            @if($makeup->experience)
                                <span class="inline-block mt-1 bg-gray-100 px-3 py-1 rounded-full text-sm text-gray-700">{{ $makeup->experience }} d'expérience</span>
                            @endif
                        </div>
                        <span class="bg-[#FADADD] px-4 py-2 rounded-lg text-[#333333] font-semibold">{{ number_format($makeup->price, 2) }} DH</span>
                    </div>
                    <p class="text-gray-700 mb-4">{{ $makeup->description ?? 'Aucune description disponible.' }}</p>

                    <div class="flex gap-2">
                        <a href="{{ route('traiteur.services.maquillage.edit', $makeup->id) }}" class="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg transition duration-300 inline-flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                            Modifier
                        </a>
                        <form action="{{ route('traiteur.services.maquillage.destroy', $makeup->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-1/3">
                <div class="bg-white rounded-xl shadow-md p-6 sticky top-6">
                    ...
                    <form action="{{ route('traiteur.services.maquillage.portfolio.store', $makeup->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
            ...
            <div class="lg:w-2/3">
                <div class="bg-white rounded-xl shadow-md p-6">
                    ...
                    @if(count($makeup->portfolioItems) > 0)
                                ...
                                <div class="portfolio-item bg-white border rounded-lg shadow-sm overflow-hidden" data-type="{{ $item->type }}">
                                    <div class="relative">
                                        ...
                                        <div class="absolute top-2 right-2 flex space-x-1">
                                            <form action="{{ route('traiteur.services.maquillage.portfolio.destroy', [$makeup->id, $item->id]) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
...
<script>
    ...
    // Confirmation de suppression du maquilleur
    function confirmDelete(button) {
        currentForm = button.closest('form');
        deleteType = 'makeup';
        document.getElementById('delete-message').textContent = 'Êtes-vous sûr de vouloir supprimer ce maquilleur ? Cette action est irréversible et supprimera aussi tout son portfolio.';

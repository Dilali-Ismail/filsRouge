<div class="mb-6">
    <h2 class="text-2xl font-semibold text-[#333333] mb-6 text-center">Gestion des Négafas</h2>

    <!-- Bouton d'ajout -->
    <div class="mb-6 flex justify-end">
        <a href="{{ route('traiteur.services.negafa.create') }}" class="px-4 py-2 bg-[#FADADD] hover:bg-[#C08081] text-[#333333] hover:text-white rounded-lg transition duration-300 shadow-md flex items-center">
                ...
                <a href="{{ route('traiteur.services.negafa.show', $negafa->id) }}" class="group">
                    <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 transform group-hover:-translate-y-1 h-full flex flex-col">
                        ...
                        <div class="relative h-48">
                            @if($negafa->photo)
                                <img src="{{ asset('storage/' . $negafa->photo) }}" alt="{{ $negafa->name }}" class="w-full h-full object-cover">
                            @else
                            ...
                            <div class="absolute top-0 right-0 mt-2 mr-2 flex gap-1">
                                <a href="{{ route('traiteur.services.negafa.edit', $negafa->id) }}" class="p-2 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-full transition duration-300" title="Modifier" onclick="event.stopPropagation()">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form action="{{ route('traiteur.services.negafa.destroy', $negafa->id) }}" method="POST" class="inline delete-form">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <!-- Informations sur la négafa -->
                        <div class="p-4 flex-grow">
                            <h3 class="text-lg font-semibold text-[#333333] mb-2 group-hover:text-[#C08081]">{{ $negafa->name }}</h3>
                            <p class="text-gray-600 text-sm mb-3 line-clamp-2">{{ $negafa->description ?? 'Aucune description' }}</p>

                            <div class="flex items-center justify-between mt-auto">
                                <span class="text-[#C08081] font-semibold">{{ number_format($negafa->price, 2) }} DH</span>
                                @if($negafa->experience)
                                    <span class="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{{ $negafa->experience }} d'expérience</span>
                                @endif
                            </div>
                        </div>
    ...
    @else
        <div class="bg-white rounded-xl shadow-md p-8 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-600 mb-4">Aucune négafa disponible</h3>
            <p class="text-gray-500 mb-6">Ajoutez votre première négafa en utilisant le bouton "Ajouter une négafa".</p>
            <a href="{{ route('traiteur.services.negafa.create') }}" class="px-5 py-3 bg-[#FADADD] hover:bg-[#C08081] text-[#333333] hover:text-white rounded-lg transition duration-300 shadow-md inline-flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
                Ajouter une négafa

...
@section('title', 'Ajouter une animation musicale')
        ...
        <div class="max-w-3xl mx-auto">
            <div class="flex items-center mb-6">
                <a href="{{ route('traiteur.services.animation.index') }}" class="mr-4 text-gray-600 hover:text-[#C08081]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                </a>
                <h1 class="text-3xl font-bold text-[#333333] font-display">Ajouter une animation musicale</h1>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-8">
                <form action="{{ route('traiteur.services.animation.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="chaabi" {{ old('type') == 'chaabi' ? 'selected' : '' }}>Chaâbi</option>
                                <option value="dakka" {{ old('type') == 'dakka' ? 'selected' : '' }}>Dakka Marrakchia</option>
                                <option value="andalouse" {{ old('type') == 'andalouse' ? 'selected' : '' }}>Musique Andalouse</option>
                                <option value="orchestre" {{ old('type') == 'orchestre' ? 'selected' : '' }}>Orchestre</option>
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="6" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FADADD] focus:border-[#FADADD]"
                                placeholder="Détaillez les caractéristiques de l'animation : nombre de musiciens, répertoire, durée de la prestation, instruments, etc.">{{ old('description') }}</textarea>

...
@section('title', 'Modifier - ' . $animation->name)
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-3xl mx-auto">
            <div class="flex items-center mb-6">
                <a href="{{ route('traiteur.services.animation.index') }}" class="mr-4 text-gray-600 hover:text-[#C08081]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                </a>
                <h1 class="text-3xl font-bold text-[#333333] font-display">Modifier l'animation musicale</h1>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-8">
                <form action="{{ route('traiteur.services.animation.update', $animation->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <label for="name" class="block text-sm font-medium text-[#333333] mb-2">
                                Nom de l'animation <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="name" id="name" value="{{ old('name', $animation->name) }}" required
                                ...
                                placeholder="Ex: Orchestre Al Andalous, Groupe Dakka Marrakchia...">
                        ...
                        <div>
                            <label for="type" class="block text-sm font-medium text-[#333333] mb-2">
                                Type d'animation <span class="text-red-500">*</span>
                            </label>
                            <select name="type" id="type" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FADADD] focus:border-[#FADADD]">
                                <option value="chaabi" {{ old('type', $animation->type) == 'chaabi' ? 'selected' : '' }}>Chaâbi</option>
                                <option value="dakka" {{ old('type', $animation->type) == 'dakka' ? 'selected' : '' }}>Dakka Marrakchia</option>
                                <option value="andalouse" {{ old('type', $animation->type) == 'andalouse' ? 'selected' : '' }}>Musique Andalouse</option>
                                <option value="orchestre" {{ old('type', $animation->type) == 'orchestre' ? 'selected' : '' }}>Orchestre</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price', $animation->price) }}" required min="0" step="0.01"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FADADD] focus:border-[#FADADD]"
                        ...
                        <div class="md:col-span-2">
                            ...
                            @if($animation->photo)
                                <div class="mb-3">
                                    ...
                                    <img src="{{ asset('storage/' . $animation->photo) }}" alt="{{ $animation->name }}" class="w-40 h-40 object-cover rounded-md">
                                </div>
                            @endif
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="6" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FADADD] focus:border-[#FADADD]"
                                placeholder="Détaillez les caractéristiques de l'animation : nombre de musiciens, répertoire, durée de la prestation, instruments, etc.">{{ old('description', $animation->description) }}</textarea>
                    ...
                    <div class="flex justify-end mt-8">
                        <a href="{{ route('traiteur.services.animation.index') }}" class="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition duration-300 shadow-md mr-3">
                            Annuler
                        </a>
